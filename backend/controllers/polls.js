...
exports.vote = asyncHandler( async (req,res,next) => {
    ...
    if(!poll) {
        return next( new ErrorRespose(`Poll is not found with pollId ${req.params.pollId}`))
    }

    const pollUser = await PollUser.findOne({user: req.user.id, poll: req.params.pollId});
    if( pollUser){
